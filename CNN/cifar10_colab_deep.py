...
def main():
    ...
    # Using num_workers=2 which works well on Google Colab
    # In PyTorch, we have to provide a cache folder. We'll use a hidden folder 
    # (~/.pytorch) so it acts exactly like the silent Keras direct link!
    trainset = torchvision.datasets.CIFAR10(root='~/.pytorch/CIFAR10_data/', train=True,
                                            download=True, transform=transform_train)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
                                              shuffle=True, num_workers=2)

    testset = torchvision.datasets.CIFAR10(root='~/.pytorch/CIFAR10_data/', train=False,
